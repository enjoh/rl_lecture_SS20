...
if __name__ == '__main__':
    ...
    times = np.zeros((len(do_nothing), episodes))
    stochastic_wind = False
    for i, v in enumerate(do_nothing):
        env = WindyGridWorld(do_nothing_allowed=v)
        state = env.reset()
        agent = SARSA(env)
    ...
    fig, ax = plt.subplots()
    for i, v in enumerate(do_nothing):
        ax.plot(times[i], label='no movement: {}'.format(v))
